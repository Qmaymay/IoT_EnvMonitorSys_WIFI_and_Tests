...
#     def test_data_collector_initialization(self, mock_mqtt_client):
#      """测试DataCollector初始化"""
#     # 直接在这个测试中mock DatabaseManager
...
#         # 验证回调函数被设置
#         assert collector.mqtt_client.on_connect is not None
#         assert collector.mqtt_client.on_message is not None
...
#         reason="需要真实的数据库文件"
#     )
#     def test_real_database_connection(self, temp_db_file):
#         """测试真实数据库连接（使用临时文件）"""
#         # 这里可以测试真实的数据库操作
#         # 暂时跳过，后续添加
