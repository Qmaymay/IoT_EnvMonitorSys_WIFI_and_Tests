...
int main(void) {
    ...
    // Initialize logger (可选，如果logger.c中有初始化函数)
    // logger_init(LOG_LEVEL_INFO);
    
    log_info("MAIN", "=== IoT Environmental Monitor System ===");
    ...
    while (1) {
        switch (state) {
            ...
            case STATE_CONNECTING_WIFI:
                log_info("MAIN", "Connecting to WiFi...");
                ...
                if (wifi_connect(WIFI_SSID, WIFI_PASSWORD) == RESULT_OK) {
                    ...
                    log_info("MAIN", "WiFi connection failed");
                    retry_count++;
                    
                    if (retry_count >= max_retries) {
                        log_info("MAIN", "Max retries reached, entering error state");
                        state = STATE_ERROR;
                    }
                }
                delay_ms(1000);
            ...
            case STATE_CONNECTING_MQTT:
                log_info("MAIN", "Connecting to MQTT server...");
                
                if (mqtt_connect(MQTT_BROKER, MQTT_PORT) == RESULT_OK) {
                    log_info("MAIN", "MQTT connected successfully");
                    state = STATE_NORMAL;
                    retry_count = 0;
                } else {
                    log_info("MAIN", "MQTT connection failed");
                    retry_count++;
                    
                    if (retry_count >= max_retries) {
                        log_info("MAIN", "Max retries reached, reconnecting WiFi");
                        state = STATE_CONNECTING_WIFI;
                    }
                }
                delay_ms(1000);
            ...
            case STATE_NORMAL:
                ...
                if (sensor_read_all(&sensor_data) == RESULT_OK) {
                    ...
                    if (mqtt_publish_data(MQTT_TOPIC, &sensor_data) != RESULT_OK) {
                        log_info("MAIN", "MQTT publish failed, reconnecting...");
                        state = STATE_CONNECTING_MQTT;
                    }
                } else {
                    log_info("MAIN", "Sensor read failed");
                }
            ...
            case STATE_ERROR:
                log_info("MAIN", "Error state, waiting for recovery...");
...
result_code_t wifi_connect(const char* ssid, const char* password) {
    log_info("WIFI", "Attempting to connect to WiFi...");
